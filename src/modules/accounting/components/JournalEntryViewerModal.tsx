...
const JournalEntryViewerModal: React.FC<JournalEntryViewerModalProps> = ({
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                const queryByReferenceType = async (candidate: string) => {
                    try {
                        const data = await db.sql`
                            SELECT * FROM journal_entry_header 
                            WHERE reference_type = ${referenceType} 
                            AND reference_id = ${candidate}
                    ...
                    for (const candidate of referenceCandidates) {
                        for (const docTypeCandidate of documentTypes) {
                            try {
                                const data = await db.sql`
                                    SELECT * FROM journal_entry_header 
                                    WHERE reference_document_id = ${candidate} 
                                    AND document_type = ${docTypeCandidate}
                                    ORDER BY posting_date DESC, created_at DESC
                                `;
                                if (data?.length) {
                                    fallbackRows = data;
            ...
            try {
                ...
                try {
                    const data = await db.sql`
                        ...
                        WHERE journal_entry_id = ${selectedEntryId}
                        ORDER BY id ASC
                    `;
                    lineRows = data;
                } catch (err: any) {
                    if (isMissingTableError(err, 'journal_entry_lines')) {
                        setLines([]);
                    ...
                    for (const candidate of referenceCandidates) {
                        for (const docTypeCandidate of documentTypes) {
                            try {
                                ...
                                `;
                                if (data?.length) {
                                    lineRows = data.filter((row: any) => String(row.journal_entry_id || '') === selectedEntryId || !row.journal_entry_id);
