...
const BalanceCarryforward: React.FC = () => {
  ...
  useEffect(() => {
    const loadConfiguration = async () => {
      try {
        const [companies, books, gls, assigns] = await Promise.all([
          db.sql`SELECT id, code, description, status FROM company_codes WHERE status = 'Active' ORDER BY code ASC`,
          db.sql`SELECT id, company_code_id, set_of_books_id, description, active_status FROM set_of_books WHERE active_status = 'Active' ORDER BY set_of_books_id ASC`,
          db.sql`SELECT id, set_of_books_id, gl_code, gl_name, gl_type, active_status, posting_count FROM gl_master WHERE active_status = 'Active' ORDER BY gl_code ASC`,
          db.sql`SELECT id, set_of_books_id FROM gl_assignments`
        ]);

        const nextCompanies = (companies || []) as CompanyCode[];
        const nextBooks = (books || []) as SetOfBook[];
        setCompanies(nextCompanies);
        setSetOfBooks(nextBooks);
        setGlMasters((gls || []) as GlMaster[]);
        setGlAssignments((assigns || []) as GlAssignment[]);
