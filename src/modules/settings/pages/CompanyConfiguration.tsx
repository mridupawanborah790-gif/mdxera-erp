...
import { RegisteredPharmacy } from '../../../../src/core/types/types';
import { db } from '../../../../src/core/database/databaseService';
import * as storage from '../../../../src/core/services/storageService';
...
const CompanyConfiguration: React.FC<CompanyConfigurationProps> = ({ currentUser }) => {
  ...
  useEffect(() => {
    const initializeStore = async () => {
      ...
      try {
        const [companies, books, glMasters, assignments, logs, history, banks] = await Promise.all([
          storage.getData('company_codes', [], currentUser),
          storage.getData('set_of_books', [], currentUser),
          storage.getData('gl_master', [], currentUser),
          storage.getData('gl_assignments', [], currentUser),
          storage.getData('setup_wizard_defaults_log', [], currentUser),
          storage.getData('gl_assignment_history', [], currentUser),
          storage.getData('bank_master', [], currentUser),
        ]);

        const booksForOrg: SetOfBooks[] = books.map((b: any) => ({
          id: b.id,
          companyCodeId: b.company_code_id || b.companyCodeId,
          setOfBooksId: b.name || b.set_of_books_id || b.setOfBooksId || '',
          description: b.description || '',
          defaultCurrency: b.default_currency || b.defaultCurrency || 'INR',
          defaultCustomerGLId: b.default_customer_gl_id || b.defaultCustomerGLId || undefined,
          defaultSupplierGLId: b.default_supplier_gl_id || b.defaultSupplierGLId || undefined,
          defaultDemoBankGLId: b.default_demo_bank_gl_id || b.defaultDemoBankGLId || undefined,
          defaultBankGLId: b.default_bank_gl_id || b.defaultBankGLId || undefined,
          activeStatus: b.active_status || b.activeStatus || 'Active',
          postingCount: b.posting_count || b.postingCount || 0,
          created_by: b.created_by || b.createdBy || SYSTEM_USER,
          created_at: b.created_at || b.createdAt || now(),
          updated_by: b.updated_by || b.updatedBy || SYSTEM_USER,
          updated_at: b.updated_at || b.updatedAt || now(),
        }));

        const normalizedCompanies: CompanyCode[] = companies.map((c: any) => {
          const rawDefaultSob = String(c.default_set_of_books_id || c.defaultSetOfBooksId || '').trim();
          // Match by ID or by the human-readable identifier (setOfBooksId/name)
          const mappedByCode = booksForOrg.find((b) => 
             (b.setOfBooksId === rawDefaultSob || b.id === rawDefaultSob) && 
             (b.companyCodeId === c.id || b.companyCodeId === c.code) && 
             b.activeStatus === 'Active'
          );

          return {
            id: c.id,
            organizationId: c.organization_id || c.organizationId,
            code: c.code,
            description: c.description || '',
            status: c.status || 'Active',
            isDefault: !!(c.is_default || c.isDefault),
            defaultSetOfBooksId: mappedByCode?.setOfBooksId || '',
            created_by: c.created_by || c.createdBy || SYSTEM_USER,
            created_at: c.created_at || c.createdAt || now(),
            updated_by: c.updated_by || c.updatedBy || SYSTEM_USER,
            updated_at: c.updated_at || c.updatedAt || now(),
          };
        });
        ...
        const dbStore: Store = {
          ...
          glMasters: glMasters.map((g: any) => ({
            id: g.id,
            setOfBooksId: g.set_of_books_id || g.setOfBooksId,
            glCode: g.gl_code || g.glCode,
            glName: g.gl_name || g.glName,
            glType: g.gl_type || g.glType,
            accountGroup: g.account_group || g.accountGroup || '',
            subgroup: g.subgroup || '',
            alias: g.alias || '',
            mappingStructure: g.mapping_structure || g.mappingStructure || '',
            postingAllowed: !!(g.posting_allowed !== undefined ? g.posting_allowed : g.postingAllowed),
            controlAccount: !!(g.control_account !== undefined ? g.control_account : g.controlAccount),
            activeStatus: g.active_status || g.activeStatus || 'Active',
            seeded_by_system: !!(g.seeded_by_system !== undefined ? g.seeded_by_system : g.seededBySystem),
            template_version: g.template_version || g.templateVersion || DEFAULT_TEMPLATE_VERSION,
            postingCount: g.posting_count || g.postingCount || 0,
            created_by: g.created_by || g.createdBy || SYSTEM_USER,
            created_at: g.created_at || g.createdAt || now(),
            updated_by: g.updated_by || g.updatedBy || SYSTEM_USER,
            updated_at: g.updated_at || g.updatedAt || now(),
          })),
          glAssignments: assignments.map((a: any) => ({
            id: a.id,
            setOfBooksId: a.set_of_books_id || a.setOfBooksId,
            assignmentScope: normalizeScope(a.assignment_scope || a.assignmentScope),
            materialMasterType: normalizeScope(a.assignment_scope || a.assignmentScope) === 'MATERIAL' ? (a.material_master_type || a.materialMasterType) : undefined,
            partyType: normalizePartyType(a.party_type || a.partyType),
            partyGroup: a.party_group || a.partyGroup || '',
            controlGL: a.control_gl_id || a.controlGL || a.control_gl || '',
            activeStatus: a.active_status || a.activeStatus || 'Active',
            inventoryGL: a.inventory_gl || a.inventoryGL || '',
            purchaseGL: a.purchase_gl || a.purchaseGL || '',
            cogsGL: a.cogs_gl || a.cogsGL || '',
            salesGL: a.sales_gl || a.salesGL || '',
            discountGL: a.discount_gl || a.discountGL || '',
            taxGL: a.tax_gl || a.taxGL || '',
            seeded_by_system: !!(a.seeded_by_system !== undefined ? a.seeded_by_system : a.seededBySystem),
            template_version: a.template_version || a.templateVersion || DEFAULT_TEMPLATE_VERSION,
            created_by: a.created_by || a.createdBy || SYSTEM_USER,
            created_at: a.created_at || a.createdAt || now(),
            updated_by: a.updated_by || a.updatedBy || SYSTEM_USER,
            updated_at: a.updated_at || a.updatedAt || now(),
          })),
          setupLogs: logs.map((l: any) => ({
            id: l.id,
            setOfBooksId: l.set_of_books_id || l.setOfBooksId,
            action: l.action,
            message: l.message,
            created_at: l.created_at || l.createdAt || now(),
            created_by: l.created_by || l.createdBy || SYSTEM_USER,
          })),
          assignmentHistory: history.map((h: any) => ({
            id: h.id,
            assignmentId: h.assignment_id || h.assignmentId,
            setOfBooksId: h.set_of_books_id || h.setOfBooksId,
            materialMasterType: h.material_master_type || h.materialMasterType,
            changed_at: h.changed_at || h.changedAt || now(),
            changed_by: h.changed_by || h.changedBy || SYSTEM_USER,
            effective_from: h.effective_from || h.effectiveFrom || now(),
            previous: h.previous_payload || h.previous || {},
            next: h.next_payload || h.next || {},
          })),
          bankMasters: banks.map((b: any) => ({
            id: b.id,
            companyCodeId: b.company_code_id || b.companyCodeId,
            linkedBankGlId: b.linked_bank_gl_id || b.linkedBankGlId || undefined,
            ...
            accountType: b.account_type || b.accountType || 'Savings',
            openingBalance: Number(b.opening_balance || b.openingBalance || 0),
            openingDate: b.opening_date || b.openingDate || '',
            defaultBank: !!(b.is_default || b.defaultBank),
            activeStatus: b.active_status || b.activeStatus || 'Active',
            created_by: b.created_by || b.createdBy || SYSTEM_USER,
            created_at: b.created_at || b.createdAt || now(),
            updated_by: b.updated_by || b.updatedBy || SYSTEM_USER,
  ...
  const onSaveConfiguration = async () => {
    ...
    try {
      if (store.companies.length > 0) {
        await storage.upsertBulkData('company_codes', store.companies.map(c => ({
          ...c,
      ...
      if (store.setOfBooks.length > 0) {
        await storage.upsertBulkData('set_of_books', store.setOfBooks.map(b => {
          ...
          return {
            ...
            name: b.setOfBooksId, // Mapping the identifier to the name column
            description: b.description,
      ...
      if (normalizedGlMasters.length > 0) {
        await storage.upsertBulkData('gl_master', normalizedGlMasters.map(g => ({
          id: g.id,
          organization_id: organizationId,
      ...
      if (store.glAssignments.length > 0) {
        ...
        if (scopedPayload.length > 0) {
          await storage.upsertBulkData('gl_assignments', scopedPayload, currentUser);
        }
      }

      if (store.setupLogs.length > 0) {
        await storage.upsertBulkData('setup_wizard_defaults_log', store.setupLogs.map(l => ({
          id: l.id,
          organization_id: organizationId,
      ...
      if (store.assignmentHistory.length > 0) {
        await storage.upsertBulkData('gl_assignment_history', store.assignmentHistory.map(h => ({
          id: h.id,
          organization_id: organizationId,
      ...
      if (store.bankMasters.length > 0) {
        await storage.upsertBulkData('bank_master', store.bankMasters.map(b => ({
          id: b.id,
          organization_id: organizationId,
          company_code_id: b.companyCodeId,
