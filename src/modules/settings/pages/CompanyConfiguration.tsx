    ...
    const initializeStore = async () => {
      ...
      try {
        ...
        const booksForOrg: SetOfBooks[] = books.map((b: any) => ({
          ...
          setOfBooksId: b.setOfBooksId,
          description: b.description || '',
          defaultCurrency: b.defaultCurrency || 'INR',
          defaultCustomerGLId: b.defaultCustomerGLId || undefined,
          defaultSupplierGLId: b.defaultSupplierGLId || undefined,
          defaultDemoBankGLId: b.defaultDemoBankGLId || undefined,
          defaultBankGLId: b.defaultBankGLId || undefined,
          activeStatus: b.activeStatus || 'Active',
          postingCount: b.postingCount || 0,
          created_by: b.createdBy || b.created_by || SYSTEM_USER,
          created_at: b.createdAt || b.created_at || now(),
          updated_by: b.updatedBy || b.updated_by || SYSTEM_USER,
          updated_at: b.updatedAt || b.updated_at || now(),
        }));

        const normalizedCompanies: CompanyCode[] = companies.map((c: any) => {
          const rawDefaultSob = String(c.defaultSetOfBooksId || '').trim();
          ...
          return {
            ...
            created_by: c.createdBy || c.created_by || SYSTEM_USER,
            created_at: c.createdAt || c.created_at || now(),
            updated_by: c.updatedBy || c.updated_by || SYSTEM_USER,
        ...
        const dbStore: Store = {
          ...
          glMasters: glMasters.map((g: any) => ({
            ...
            subgroup: g.subgroup || '',
            alias: g.alias || '',
            mappingStructure: g.mappingStructure || '',
            postingAllowed: !!g.postingAllowed,
            controlAccount: !!g.controlAccount,
            activeStatus: g.activeStatus || 'Active',
            seeded_by_system: !!g.seededBySystem || !!g.seeded_by_system,
            template_version: g.templateVersion || g.template_version || DEFAULT_TEMPLATE_VERSION,
            postingCount: g.postingCount || 0,
            created_by: g.createdBy || g.created_by || SYSTEM_USER,
            created_at: g.createdAt || g.created_at || now(),
            updated_by: g.updatedBy || g.updated_by || SYSTEM_USER,
            updated_at: g.updatedAt || g.updated_at || now(),
          })),
          glAssignments: assignments.map((a: any) => ({
            id: a.id,
            setOfBooksId: a.setOfBooksId,
            assignmentScope: normalizeScope(a.assignmentScope),
            materialMasterType: normalizeScope(a.assignmentScope) === 'MATERIAL' ? a.materialMasterType : undefined,
            partyType: normalizePartyType(a.partyType),
            partyGroup: a.partyGroup || '',
            controlGL: a.controlGlId || a.controlGL || '',
            activeStatus: a.activeStatus || 'Active',
            inventoryGL: a.inventoryGl || a.inventoryGL || '',
            purchaseGL: a.purchaseGl || a.purchaseGL || '',
            cogsGL: a.cogsGl || a.cogsGL || '',
            salesGL: a.salesGl || a.salesGL || '',
            discountGL: a.discountGl || a.discountGL || '',
            taxGL: a.taxGl || a.taxGL || '',
            seeded_by_system: !!a.seededBySystem || !!a.seeded_by_system,
            template_version: a.templateVersion || a.template_version || DEFAULT_TEMPLATE_VERSION,
            created_by: a.createdBy || a.created_by || SYSTEM_USER,
            created_at: a.createdAt || a.created_at || now(),
            updated_by: a.updatedBy || a.updated_by || SYSTEM_USER,
            updated_at: a.updatedAt || a.updated_at || now(),
          })),
          setupLogs: logs.map((l: any) => ({
            id: l.id,
            setOfBooksId: l.setOfBooksId,
            action: l.action,
            message: l.message,
            created_at: l.createdAt || l.created_at || now(),
            created_by: l.createdBy || l.created_by || SYSTEM_USER,
          })),
          assignmentHistory: history.map((h: any) => ({
            id: h.id,
            assignmentId: h.assignmentId,
            setOfBooksId: h.setOfBooksId,
            materialMasterType: h.materialMasterType,
            changed_at: h.changedAt || h.changed_at || now(),
            changed_by: h.changedBy || h.changed_by || SYSTEM_USER,
            effective_from: h.effectiveFrom || h.effective_from || now(),
            previous: h.previousPayload || h.previous || {},
            next: h.nextPayload || h.next || {},
          })),
          bankMasters: banks.map((b: any) => ({
            id: b.id,
            companyCodeId: b.companyCodeId,
            linkedBankGlId: b.linkedBankGlId || undefined,
            bankName: b.bankName || '',
