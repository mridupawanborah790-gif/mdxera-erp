...
import type { AppConfigurations, ModuleConfig, InvoiceNumberConfig, DiscountRule, SlabRule, InventoryItem, Transaction, Purchase, RegisteredPharmacy, Medicine, Distributor, SupplierProductMap, Customer } from '../../../../src/core/types/types';
import * as storage from '../../../../src/core/services/storageService';
import { db } from '../../../../src/core/database/databaseService';
import { configurableModules, MASTER_SHORTCUT_OPTIONS } from '../../../../src/core/utils/constants';
import { 
...
import PurchaseBillImportPreviewModal from '../../../../src/modules/purchases/components/PurchaseBillImportPreviewModal';
import SalesBillImportPreviewModal from '../../../../src/modules/sales/components/SalesBillImportPreviewModal';
import Modal from '../../../../src/core/components/Modal';
import MasterDataMigrationWizard from '../../../../src/modules/inventory/components/MasterDataMigrationWizard';
import { fuzzyMatch } from '../../../../src/core/utils/search';
import { getFinancialYearLabel } from '../../../../src/core/utils/invoice';
import { normalizeStockHandlingConfig } from '../../../../src/core/utils/stockHandling';
...
const ConfigurationPage: React.FC<ConfigurationPageProps> = ({ 
        ...
        if (activeSection === 'invoiceNumbering' && currentUser) {
            const fetchLiveSequences = async () => {
                ...
                for (const [configKey, docType] of Object.entries(voucherKeysMap)) {
                    try {
                        const res = await storage.reserveVoucherNumber(docType, currentUser, true);
                        if (res) {
                            results[configKey] = {
                                currentNumber: res.usedNumber,
    ...
    const runDemoMigrationRpcWithFallback = async () => {
        try {
            // Local SQLite migration simulation
            const sourceData = await db.sql`SELECT * FROM material_master_migration`;
            let imported_count = 0;
            let updated_count = 0;
            let skipped_count = 0;

            for (const row of sourceData) {
                const existing = await db.sql`SELECT id FROM material_master WHERE material_code = ${row.material_code} OR name = ${row.name} LIMIT 1`;
                if (existing.length > 0) {
                    if (duplicateHandlingMode === 'UPDATE') {
                        await storage.saveData('material_master', { ...row, id: existing[0].id }, currentUser, true);
                        updated_count++;
                    } else {
                        skipped_count++;
                    }
                } else {
                    await storage.saveData('material_master', row, currentUser, false);
                    imported_count++;
                }
            }
            ...
            return { 
                data: { 
                    ...
                    updated_count, 
                    skipped_count, 
                    found_count: sourceData.length,
                    ready_count: imported_count + updated_count,
                    duplicates_count: sourceData.length - imported_count
                }, 
                error: null 
            };
        } catch (e: any) {
    ...
    const previewDefaultDemoMigration = async () => {
        try {
            const data = await db.sql`SELECT * FROM material_master_migration LIMIT 100`;
