...
import type { Supplier } from '../../../core/types/types';
import { getOutstandingBalance } from '../../../core/utils/helpers';

declare const XLSX: any;
declare const html2pdf: any;

interface ExportSuppliersModalProps {
    isOpen: boolean;
    onClose: () => void;
    data: Supplier[];
    pharmacyName: string;
...
const ExportSuppliersModal: React.FC<ExportSuppliersModalProps> = ({ isOpen, onClose, data, pharmacyName }) => {
    const headers = ['S.No', 'Name', 'Address', 'Phone', 'GSTIN', 'Outstanding Balance', 'Status'];
    ...
    const handleExportExcel = () => {
        ...
        XLSX.writeFile(wb, `Suppliers_Master_${new Date().toISOString().split('T')[0]}.xlsx`);
        onClose();
    };
    ...
    const handleExportCsv = () => {
        ...
        downloadCsv(csvContent, `Suppliers_Master_${new Date().toISOString().split('T')[0]}.csv`);
        onClose();
    };
    ...
    const handleExportPdf = () => {
        ...
        element.innerHTML = `
            <div style="text-align: center; margin-bottom: 20px;">
                ...
                <h2 style="margin: 5px 0; color: #666;">SUPPLIER MASTER (ACCOUNTS PAYABLE)</h2>
            ...
            <div style="margin-top: 20px; text-align: right; font-size: 10px; font-weight: bold;">
                Total Payables: ₹${data.reduce((s, i) => s + getOutstandingBalance(i), 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
            </div>
        `;

        const opt = {
            margin: 10,
            filename: `Suppliers_Master_${new Date().toISOString().split('T')[0]}.pdf`,
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Export Supplier Master" widthClass="max-w-md">
            <div className="p-8 space-y-6 bg-app-bg">
                <p className="text-xs font-black text-gray-500 uppercase tracking-widest text-center">
                    Select export format for Supplier Directory
                </p>
...
export default ExportSuppliersModal;
