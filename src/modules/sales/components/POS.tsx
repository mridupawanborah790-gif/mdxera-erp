...
import SchemeCalculatorModal from './SchemeCalculatorModal';
import Modal from '../../../core/components/Modal';
import AddMedicineModal from '../../inventory/components/AddMedicineModal';
import AddCustomerModal from './AddCustomerModal';
import SearchableDropdown from '../../../core/components/SearchableDropdown';
import BatchSelectionModal from '../../inventory/components/BatchSelectionModal';
import WebcamCaptureModal from '../../../core/components/WebcamCaptureModal';
import CustomerSearchModal from './CustomerSearchModal';
import JournalEntryViewerModal from '../../accounting/components/JournalEntryViewerModal';
import ProductInsightsPanel from '../../inventory/components/ProductInsightsPanel';
...
import { InventoryItem, Customer, Transaction, BillItem, AppConfigurations, RegisteredPharmacy, Medicine, Purchase, FileInput, MasterPriceMaintainRecord, SalesChallan, DoctorMaster, OrganizationMember } from '../../../core/types/types';
import { handleEnterToNextField } from '../../../core/utils/navigation';
import { fuzzyMatch } from '../../../core/utils/search';
import { formatExpiryToMMYY, getOutstandingBalance, parseNumber, checkIsExpired } from '../../../core/utils/helpers';
...
const POS = forwardRef<any, POSProps>(({
    ...
    const fetchStats = useCallback(async () => {
        ...
        try {
            ...
            const allData = await db.sql`
                SELECT total, date, status, customer_id 
                FROM sales_bill 
            ...
            if (selectedCustomer?.id) {
                filtered = filtered.filter(s => s.customer_id === selectedCustomer.id);
            }

            const monthTotal = filtered.reduce((sum, s) => sum + (Number(s.total) || 0), 0);
            const todayTotal = filtered.filter(s => s.date.startsWith(todayStr)).reduce((sum, s) => sum + (Number(s.total) || 0), 0);
            ...
            let recent = (recentRows || []) as any[];
            if (selectedCustomer?.id) {
                recent = recent.filter(s => s.customer_id === selectedCustomer.id);
            }

            setSalesHistory(recent.map(r => storage.fromDb('sales_bill', r)));
        } catch (e) {
    ...
    const handleOpenRecentSalePreview = useCallback(async (sale: Transaction) => {
        ...
        try {
            const data = await storage.getDataById<Transaction>('sales_bill', sale.id, currentUser);

            if (!data) throw new Error('Invoice not found');
            setViewingHistorySale(data);
        } catch (error) {
