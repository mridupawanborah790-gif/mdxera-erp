...
import type { RegisteredPharmacy, MbcCard, MbcCardHistory, MbcCardTemplate, MbcCardType } from '../../../core/types/types';
...
const MbcCardManagement: React.FC<Props> = ({ currentUser, activeScreen, onNavigate }) => {
  ...
  const refreshAll = async () => {
    ...
    try {
      const [typeRes, templateRes, cardsRes, historyRes] = await Promise.all([
        storage.getData('mbc_card_types', [], currentUser),
        storage.getData('mbc_card_templates', [], currentUser),
        storage.getData('mbc_cards', [], currentUser),
        storage.getData('mbc_card_history', [], currentUser),
      ]);
      setCardTypes(typeRes || []);
      setTemplates(templateRes || []);
      setCards(cardsRes || []);
      setHistory(historyRes || []);
    } catch (error) {
      console.error('Error refreshing MBC data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveCardType = async () => {
    ...
    await storage.saveData('mbc_card_types', payload, currentUser, !!typeForm.id);
  ...
  const saveTemplate = async () => {
    ...
    await storage.saveData('mbc_card_templates', {
      ...templateForm,
      organization_id: currentUser.organization_id,
      updated_at: new Date().toISOString(),
    }, currentUser, !!templateForm.id);
  ...
  const saveCard = async () => {
    ...
    const savedCard = await storage.saveData('mbc_cards', payload, currentUser, !!cardForm.id);

    await storage.saveData('mbc_card_history', {
      organization_id: currentUser.organization_id,
      mbc_card_id: savedCard.id,
  ...
  const doRenewUpgrade = async (mode: 'renew' | 'upgrade') => {
    ...
    await storage.saveData('mbc_cards', patch, currentUser, true);
                    ...
                    return (
                      <tr key={card.id} className="border-t">
                        ...
                        <td className="p-2 whitespace-nowrap">
                          ...
                          <button className="px-2 py-1 border" onClick={async () => { await storage.saveData('mbc_cards', { ...card, status: 'inactive' }, currentUser, true); refreshAll(); }}>Deactivate</button>
                        </td>
                      </tr>
                    );
                  })}
