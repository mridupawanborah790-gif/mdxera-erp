...
import type { NavItem, RegisteredPharmacy, AppConfigurations } from '@core/types';
import { settingsNavigation } from '@core/utils/constants';
...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, onNavigate, navigationItems, configurations, onToggleMasterExplorer, brandName, isKeyboardActive = true }) => {
  ...
  return (
    <div 
      ref={sidebarRef}
      className={`transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'w-16' : expandedSidebarWidthClass} bg-gray-100 flex-col h-full shrink-0 print:hidden border-r border-gray-400 z-10 hidden lg:flex`}
    >
