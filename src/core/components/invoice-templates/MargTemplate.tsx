...
import type { DetailedBill, InventoryItem, AppConfigurations, BillItem } from '../../types/types';
...
// ─── Inline BankDetails component ───────────────────────────────────────────
const BankDetailsInline = ({ bankName, accountNumber, ifscCode, className }: { bankName?: string; accountNumber?: string; ifscCode?: string; className?: string }) => (
  <div className={className}>
    {bankName && <p><span className="font-bold">BANK:</span> {bankName}</p>}
    {accountNumber && <p><span className="font-bold">A/C NO:</span> {accountNumber}</p>}
    {ifscCode && <p><span className="font-bold">IFSC:</span> {ifscCode}</p>}
  </div>
);
...
const MargTemplate: React.FC<TemplateProps> = ({ bill, orientation = 'portrait' }) => {
  ...
  const { items, gstSummary, subtotalValue, totalGst, roundOff, grandTotal,
    ...
    const items = (bill.items || []).map((item: BillItem) => {
      const inventoryItem = bill.inventory?.find((inv: InventoryItem) => inv.id === item.inventoryItemId);
    ...
    const schemeDiscount = (bill.items || []).reduce((s: number, it: BillItem) => s + Number(it.schemeDiscountAmount || 0), 0);
  ...
  const pages = useMemo(() => {
    ...
    return indices.map((idxArr: number[]) => idxArr.map((i: number) => items[i]));
  }, [caps, items]);
  ...
  const ItemRow = ({ item, serial }: { item: any; serial: number }) => (
    <tr key={item.id}>
