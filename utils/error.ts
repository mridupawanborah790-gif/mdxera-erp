...
export const parseNetworkAndApiError = (error: any): string => {
    ...
    if (message.includes('model is not available') || message.includes('unsupported model') || message.includes('not found for api version')) {
        return "Selected Gemini model is unavailable for this key/project. Set a supported model via VITE_GEMINI_MODEL (for example gemini-2.0-flash), and ensure Gemini API is enabled for the same Google project as your key.";
    }
