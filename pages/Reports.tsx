    ...
    const generateReportData = (reportId: string) => {
        ...
        switch (reportId) {
            ...
            case 'productWiseSales':
                ...
                applyDateFilter(transactions).forEach(tx => {
                    (tx.items || []).forEach((item: any) => {
                        ...
                        const saleAmount = (item.quantity || 0) * (item.rate || item.mrp || 0) * (1 - (item.discountPercent || 0) / 100);
            ...
            case 'stockSalesAnalysis':
                ...
                applyDateFilter(transactions).forEach(tx => {
                    (tx.items || []).forEach((item: any) => {
                        salesByProduct.set(item.name, (salesByProduct.get(item.name) || 0) + (item.quantity || 0));
                    });
                });
                filteredData = inventory.map((item: InventoryItem) => {
                ...
                }).sort((a,b) => b['Stock Turnover'] - a['Stock Turnover']);
                break;
            default:
