...
import { Customer, RegisteredPharmacy } from '../types';
...
interface AccountReceivableProps {
    customers: Customer[];
    onRecordPayment: (customerId: string, amount: number, date: string, description: string) => void;
    currentUser: RegisteredPharmacy | null;
}

const AccountReceivable: React.FC<AccountReceivableProps> = ({ customers, onRecordPayment, currentUser }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await onRecordPayment(selectedCustomer.id, Number(amount), date, description);
                    ...
                    {selectedCustomer ? (
                        <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-8">
                            <div className="pb-4 border-b border-gray-300">
                                ...
                                <h2 className={`${uniformTextStyle} !text-4xl text-primary`}>{selectedCustomer.name}</h2>
                                <div className="mt-4 flex gap-4 text-xs font-black uppercase">
                                    <span className="text-gray-400">Total Outstanding: <span className="text-red-600 text-lg">₹{getOutstandingBalance(selectedCustomer).toFixed(2)}</span></span>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 gap-6">
                                <div>
                                    <label className="text-xs font-black text-gray-500 uppercase block mb-2 tracking-widest">Receipt Amount (₹)</label>
                                    <input 
                                        type="number" 
                                        required
                                        autoFocus
                                        ...
                                        className="w-full border-4 border-gray-400 p-6 text-5xl font-black focus:bg-yellow-50 focus:border-primary outline-none text-emerald-700 no-spinner shadow-inner"
                                        placeholder="0.00"
                                    />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Receipt Date</label>
                                        <input 
                                            type="date" 
                                            required
                                            value={date}
                            ...
                            <div className="pt-6 flex justify-end gap-4">
                                <button type="button" tabIndex={-1} onClick={() => setSelectedCustomer(null)} className="px-8 py-3 tally-border bg-white font-black uppercase text-xs hover:bg-gray-50 transition-colors">Discard</button>
                                <button 
                                    type="submit"
                                    tabIndex={-1}
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center opacity-30">
                            <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mb-4"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="10" y1="22" x2="22" y2="10"/><line x1="7" y1="15" x2="7.01" y2="15"/><line x1="11" y1="15" x2="11.01" y2="15"/></svg>
                            <p className="text-2xl font-black uppercase tracking-[0.2em]">Select Ledger to post Receipt</p>
                            <p className="text-sm font-bold mt-2">Search and click on a debtor from the left panel</p>
                        </div>
                    )}
                </Card>
            </div>
