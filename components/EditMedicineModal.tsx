...
const EditMedicineModal: React.FC<EditMedicineModalProps> = ({ isOpen, onClose, onSave, medicine, organizationType, existingMedicines = [] }) => {
    ...
    const validate = useCallback(() => {
        ...
        if ((formState.valuationMethod || 'moving_average') === 'standard' && Number(formState.standardPriceRate || 0) < 0) newErrors.standardPriceRate = 'Standard Price Rate cannot be negative.';
                    ...
                    <div className="bg-gray-50 p-4 border border-gray-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="standardPriceRate" value={Number(formState.standardPriceRate || 0)} onChange={handleChange} disabled={(formState.valuationMethod || 'moving_average') !== 'standard'} className="w-full p-2 border border-gray-400 font-bold text-sm bg-input-bg disabled:bg-gray-100 disabled:text-gray-500 outline-none" />
                            </div>
                            <div>
