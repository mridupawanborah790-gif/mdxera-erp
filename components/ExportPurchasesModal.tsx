...
interface ExportPurchasesModalProps {
    isOpen: boolean;
    onClose: () => void;
    data: Purchase[];
    pharmacyName: string;
...
const ExportPurchasesModal: React.FC<ExportPurchasesModalProps> = ({ isOpen, onClose, data, pharmacyName }) => {
    const headers = ['S.No', 'System ID', 'Invoice No', 'Date', 'Supplier', 'Subtotal', 'Tax (GST)', 'Total Amount', 'Status'];
    ...
    const handleExportCsv = () => {
        ...
        downloadCsv(csvContent, `Purchase_Register_${new Date().toISOString().split('T')[0]}.csv`);
        onClose();
    };
    ...
    const handleExportPdf = () => {
        ...
        element.innerHTML = `
            <div style="text-align: center; margin-bottom: 20px;">
                ...
                <h2 style="margin: 5px 0; color: #666;">PURCHASE REGISTER (INWARD)</h2>
            ...
            <div style="margin-top: 20px; text-align: right; font-size: 10px; font-weight: bold;">
                Total Purchase: ₹${data.reduce((s, p) => s + (p.status !== 'cancelled' ? (p.totalAmount || 0) : 0), 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
            </div>
        `;

        const opt = {
            margin: 10,
            filename: `Purchase_Register_${new Date().toISOString().split('T')[0]}.pdf`,
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Export Purchase Register" widthClass="max-w-md">
            <div className="p-8 space-y-6 bg-app-bg">
                <p className="text-xs font-black text-gray-500 uppercase tracking-widest text-center">
                    Select export format for inward bills
                </p>
...
export default ExportPurchasesModal;
