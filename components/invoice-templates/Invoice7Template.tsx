...
function paginateItems(items: BillItem[]) {
...
const Invoice7Template: React.FC<TemplateProps> = ({ bill }) => {
  ...
  const paginatedItems = useMemo(() => {
    ...
    return paginateItems(allItems).filter((pageItems) => pageItems && pageItems.length > 0);
  }, [billDetails.items]);
        ...
        return (
          <div key={`invoice-7-page-${pageIndex}`} className="invoice-page leading-[1.2]">
            <div className="header">
              ...
              <div className="text-right text-[7px] mt-0.5">
                Page {pageIndex + 1} of {paginatedItems.length}
              </div>
